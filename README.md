...
- **例外2**：与产品咨询完全无关的闲聊/荒谬提问（"吃饭了嘛""这产品对国家安全有危害吗"之类）。这类说法千变万化，没法像打招呼一样靠关键词穷举，改用一次轻量 AI 判断：只有 AI **非常确信**完全无关时才直接回复引导语，不落库、不进队列、不发邮件；只要有一点点可能与产品相关，就判定为"相关"、正常走检索/转人工流程，避免真实问题被误判漏单。管理员可在工作台的「无关闲聊自动过滤」中随时关闭（对应 `skip_irrelevant_enabled` 设置，默认开启）。
