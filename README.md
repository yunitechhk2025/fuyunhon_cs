...
邮件通过标准 SMTP 发送，发信账号/服务器信息在 `.env` 的 `SMTP_*` 变量中配置（见 `.env.example`）；未配置 `SMTP_HOST` 时会自动跳过发送并只打印警告日志，不影响客户提问、客服回复等主流程。

收件邮箱默认取 `.env` 里的 `NOTIFY_EMAIL_TO`，管理员也可以登录客服工作台后在「工作台设置 → 邮件提醒收件人」里直接修改（支持逗号分隔多个收件人），后台配置优先于 `.env`，改动立即生效、无需重启服务。
...
- **工作台设置**（右上角按钮，弹出面板）：管理员可在此配置邮件提醒收件人、调整人机协同的自动发送超时时间、开启/关闭待处理队列定时提醒邮件并设置提醒间隔（分钟），以及管理客服账号。
