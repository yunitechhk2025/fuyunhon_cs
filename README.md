...
- **例外2**：与产品咨询完全无关的闲聊/荒谬提问，或明显不构成真实客服需求的挑逗性提问（"吃饭了嘛""哪里买到假货"之类，后者特指字面上问"哪里能买到假货本身"，跟"怎么辨别真伪""在哪买正品才不会买到假货"这类正常防伪疑虑完全不同、后者仍会正常判定为相关）。这类说法千变万化，没法像打招呼一样靠关键词穷举，改用一次轻量 AI 判断（`_classify_irrelevant`）：只有 AI **非常确信**完全无关/不构成真实需求时才直接回复引导语；这类问题仍会完整落库（标记为 `matched=True`、`matched_question="无关闲聊/非常规提问"`，跟问候语的记录方式一致），方便管理员事后能在对话记录里查到，只是不进人工队列、不发邮件、不计入"转人工请求次数"统计；只要有一点点可能与产品相关，就判定为"相关"、正常走检索/转人工流程，避免真实问题被误判漏单。管理员可在工作台的「无关闲聊自动过滤」中随时关闭（对应 `skip_irrelevant_enabled` 设置，默认开启）。
  - **重要例外（不属于"无关闲聊"）**：涉及低俗/色情、不道德、国家安全、政治敏感、暴力、违禁品等严肃或敏感内容的提问，即使表面上跟产品毫无关系，也**不会**被这个过滤器判定为无关闲聊——`_classify_irrelevant` 的 system prompt 里明确要求这类内容一律判定为相关（`irrelevant: false`），正常走检索/转人工流程，确保人工客服能实际看到并介入处理，不会被 AI 一句引导语悄悄打发掉（此前的 prompt 曾把"这产品对国家安全有危害吗"当作无关闲聊的示例，属于已知的错误示范，现已修正并从示例中移除）。
