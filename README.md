...
邮件通过标准 SMTP 发送，相关账号/服务器信息在 `.env` 的 `SMTP_*` 变量中配置（见 `.env.example`）；未配置 `SMTP_HOST` 时会自动跳过发送并只打印警告日志，不影响客户提问、客服回复等主流程。测试阶段默认收件邮箱是 `NOTIFY_EMAIL_TO`（可在 `.env` 中修改，支持逗号分隔多个收件人）。
...
- **工作台设置**（右上角按钮，弹出面板）：管理员可在此调整人机协同的自动发送超时时间、开启/关闭待处理队列定时提醒邮件并设置提醒间隔（分钟），以及管理客服账号。
